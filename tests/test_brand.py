...
def test_scale_words_multiply_only_their_group():
    assert normalize_numbers("nine thousand five hundred players") == "9500 players"
    assert normalize_numbers("one thousand") == "1000"
    assert normalize_numbers("two million") == "2000000"


def test_lone_number_words_stay_english():
    """'one'/'two' as ordinary words must not become claimable digits."""
    assert normalize_numbers("the one that doesn't melt") == "the one that doesn't melt"
    assert normalize_numbers("two bags. one standard.") == "two bags. one standard."
    # ...but a lone number in numeric context still converts
    assert normalize_numbers("five dollars off") == "$5 off"
    assert normalize_numbers("fifty percent") == "50%"


def test_hook_copy_without_claims_is_not_flagged():
    assert lint_edp(_edp(callout="THE ONE THAT DOESN'T *MELT*"), KIT) == []
    assert lint_edp(_edp(vo="Two bags. One standard. Never settle."), KIT) == []
...
def test_banned_word_and_em_dash():
    issues = lint_edp(_edp(vo="The best weekender bag.", callout="PREMIUM — DIALED"), KIT)
