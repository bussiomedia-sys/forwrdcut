...
```jsonc
{
  ...
  "segments": [
    ...
    {"source": "clip.mp4", "in": 2.5, "out": 5.5, "role": "vo",
     "reframe": "cover", "voiceover": "Most weekender bags are an afterthought.",
  ...
  "overlays": [
    {"type": "callout", "text": "840D *BALLISTIC* NYLON", "position": "lower", "start": 8.1, "end": 11.4, "fade": 0.3},
    {"type": "logo",    "path": "assets/brand/logo.png", "position": "safe_upper", "width_frac": 0.42, "start": 0.3, "end": 5.0, "fade": 0.4},
    ...
    {"type": "cta",     "text": "SHOP NOW · EXAMPLE.COM", "position": "lower", "start": 27, "end": 32}
  ],
...
- `callout` — big Inter-Black all-caps headline, white with one `*orange word*` (mark a
  **single** word with asterisks), word-wrapped. A soft drop shadow (not a heavy keyline) keeps
  it legible over footage while reading premium. Options: `size_frac`/`max_width_frac` (fractions
  of frame width), `emphasis_style` (`"color"` orange word or `"chip"` filled pill for busy/very
  light backgrounds), `upper: false` (keep sentence case), `weight`.
- `logo` — composite a real brand PNG (script wordmark, emblem…) rather than a text pill.
  `path`, `width_frac` (of frame width), `shadow: true` (soft shadow to hold over footage).
- `cta` — orange pill button. `badge` — small brand/credibility chip. `stars` — rating row
  + text. `progress` — thin retention bar.
- Every overlay takes `start`/`end` (absolute seconds), `position`, and optional **`fade`**
  (seconds) for a gentle alpha ease-in/out instead of a hard pop.
- **Positions:** `top-left` · `top-right` · `upper` · `center` · `lower`, plus **`safe_upper`
  (0.20)** and **`safe_lower` (0.62)** — these keep the element inside the centre band that
  survives a 4:5 or 1:1 auto-crop of a 9:16 master, so one 9:16 render is safely **multi-use**
  across Meta placements. Compose the wordmark, product, and headline inside that band.
