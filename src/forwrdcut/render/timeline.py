...
# Brand display spellings — Whisper mishears brand/product names; map the misheard forms to
# the on-screen spelling here (e.g. {"acme": "ACME"}). Empty by default: the engine ships
# brand-neutral, so add your own or drive them from a brand kit.
_BRAND_CAPTION: dict[str, str] = {}


def _normalize_caption_words(words: list[dict]) -> list[dict]:
    """Re-join hyphenated compounds Whisper splits ("30"+"-day" -> "30-day") and apply any
    brand display spellings from ``_BRAND_CAPTION`` (empty by default)."""
    import re
    ...
    while i < n:
        w = words[i]
        if i + 1 < n and words[i + 1]["word"].startswith("-") and any(c.isalnum() for c in words[i + 1]["word"]):
...
_SEG_CACHE_SALT = "seg-v2"      # bump when segment-render semantics change


# Stream properties that must match across parts for `concat -c copy` to be lossless.
_CONCAT_PROPS = ("codec_name", "width", "height", "pix_fmt",
                 "sample_aspect_ratio", "time_base")


def _parts_are_concat_compatible(parts: list[Path]) -> bool:
    """True when every part shares the stream params `-c copy` concat requires.

    ffmpeg will happily *succeed* on a mismatched copy-concat and emit a file with beats
    missing, so this has to be checked rather than caught."""
    import subprocess
    sigs = set()
    for p in parts:
        r = subprocess.run(
            ["ffprobe", "-v", "error", "-select_streams", "v:0", "-show_entries",
             "stream=" + ",".join(_CONCAT_PROPS), "-of", "compact=p=0:nk=1", str(p)],
...
def render_timeline(edp: dict, out_path: str | Path, cfg: Config | None = None,
    ...
    if xtype and xtype not in ("cut", "none") and len(parts) > 1:
        ...
        # `-c copy` concat is only valid when every part shares the same stream params.
        # When they differ (classically pix_fmt yuvj420p from a JPEG still vs yuv420p from
        # a video) ffmpeg does NOT fail — it silently drops beats and holds a frame. So we
        # check compatibility up front, and verify the assembled duration afterwards.
        if not _parts_are_concat_compatible(parts):
            _reencode_concat()
        else:
            listfile = tmpdir / "list.txt"
            listfile.write_text("".join(f"file '{p.as_posix()}'\n" for p in parts))
            expected = sum(probe(p).duration or 0.0 for p in parts)
            try:
                run(["-f", "concat", "-safe", "0", "-i", str(listfile), "-c", "copy",
                     "-movflags", "+faststart", str(concat_out)], desc="concat (copy)")
                got = probe(concat_out).duration or 0.0
                if abs(got - expected) > 0.15:
                    raise RuntimeError(
                        f"copy-concat lost content: {got:.2f}s vs expected {expected:.2f}s")
            except Exception:
                _reencode_concat()
    ...
    if not look and not specs:
        ...
        venc = pick_video_encoder(rcfg.get("hw_encoder", "h264_videotoolbox"),
                                  rcfg.get("sw_encoder", "libx264"))
        # The video look/overlay filtergraph and the loudnorm audio filter must NOT share
        # one filter_complex. When they do (with the hardware encoder and `-shortest`), the
        # audio branch starves the chained-overlay framesync and whole video beats get
        # dropped and held — a silent corruption the render exits 0 on. So: burn the visuals
        # here with audio copied through, then loudnorm in a separate audio-only pass below.
        vpass = (tmpdir / "_visual.mp4") if loudnorm else out_path
        args = ["-i", str(current)]
        chain, lab = [], "0:v"
        if look:
        ...
        for i, s in enumerate(specs, start=1):
            ...
            # Optional alpha fade in/out on the overlay's own stream timeline (which starts
            # at 0, aligned to the main clock) — a premium ease instead of a hard pop.
            fd = float(s.get("fade") or 0.0)
            ov_pre = "format=rgba"
            if fd > 0 and s.get("start") is not None:
                st = float(s["start"])
                ov_pre += f",fade=t=in:st={st:.3f}:d={fd:.3f}:alpha=1"
                if s.get("end") is not None:
                    out_st = max(st, float(s["end"]) - fd)
                    ov_pre += f",fade=t=out:st={out_st:.3f}:d={fd:.3f}:alpha=1"
            chain.append(f"[{i}:v]{ov_pre}[ov{i}]")
            chain.append(f"[{lab}][ov{i}]overlay=x={s['x']}:y={s['y']}:eval=frame{en}[{o}]")
            lab = o
        if lab != "vout":
            chain.append(f"[{lab}]format=yuv420p[vout]")
        args += ["-filter_complex", ";".join(chain), "-map", "[vout]", "-map", "0:a?",
                 "-c:a", "copy", "-c:v", venc, "-b:v", str(rcfg.get("video_bitrate", "10M")),
                 "-pix_fmt", "yuv420p", "-shortest", "-movflags", "+faststart", str(vpass)]
        run(args, desc="finish (look+overlays, video pass)")
        if loudnorm:
            # audio-only encode: video copied byte-for-byte, so the timeline is untouched
            run(["-i", str(vpass), "-af", _LOUDNORM, "-c:v", "copy",
                 "-c:a", "aac", "-b:a", str(rcfg.get("audio_bitrate", "192k")),
                 "-movflags", "+faststart", str(out_path)], desc="loudnorm (audio pass)")
